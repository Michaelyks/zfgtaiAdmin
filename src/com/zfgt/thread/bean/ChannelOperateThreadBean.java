...
import com.zfgt.orm.ErrorRecord;
...
public class ChannelOperateThreadBean {
	...
	public void addChannelOperateInvest(String yesterdays) {
		...
		sb.append(
				"INSERT INTO channel_operate_day ( date, registChannel, investUserSum, firstInvestUserSum, reInvestUserSum, investCount, "
						...
						+ "reInvestCentSum ) SELECT z.date, z.registChannel, IFNULL(s.investUserSum, 0), IFNULL(s.firstInvestUserSum, 0), "
						...
						+ "ifnull(s.reInvestCentSum, 0) FROM ( SELECT DATE_FORMAT(x.insert_time, '%Y-%m-%d') date, y.channel registChannel FROM dateday X, plat_channel Y  ) z "
						...
						+ "GROUP BY DATE_FORMAT(a.pay_time, '%Y-%m-%d'), c.regist_channel ) s ON z.date = s.investDate AND z.registChannel = s.registChannel "
		...
		sb = new StringBuffer();
		sb.append(
				"INSERT INTO channel_operate_day ( date, registChannel, regInvestCount, regInvestUserSum, regInvestCentSum ) "
						+ "SELECT z.date, z.registChannel, IFNULL(s.regInvestCount, 0), IFNULL(s.regInvestUserSum, 0), IFNULL(s.regInvestCentSum, 0) "
						+ "FROM ( SELECT DATE_FORMAT(x.insert_time, '%Y-%m-%d') date, y.channel registChannel FROM dateday x, plat_channel y ) z "
						+ "LEFT JOIN "
		...
		sb = new StringBuffer();
		sb.append(
				" INSERT INTO channel_operate_day ( DATE, registChannel, newTwoInvestCount, newTwoInvestUserSum, newTwoInvestCentSum ) " +
				" SELECT z.date, z.registChannel, IFNULL(s.newTwoInvestCount, 0), IFNULL(s.newTwoInvestUserSum, 0), IFNULL(s.newTwoInvestCentSum, 0) " +
				" FROM ( SELECT DATE_FORMAT(x.insert_time, '%Y-%m-%d') DATE, y.channel registChannel FROM dateday X,  plat_channel Y ) z" +
				" LEFT JOIN " +
				" ( " +
	...
	public void addChannelOperateRegist(String yesterdays) {
		...
		sb.append(
				"INSERT INTO channel_operate_day ( date, registChannel, regUserSum)"
						+" SELECT z.date, z.registChannel, "
						+" ifnull(s.regUserSum, 0) regUserSum FROM ( SELECT DATE_FORMAT(x.insert_time, '%Y-%m-%d') date, "
						+" y.channel registChannel FROM dateday x,  plat_channel y ) z"
						+" LEFT JOIN "
						...
						+" AND z.registChannel = s.registChannel ON DUPLICATE KEY UPDATE regUserSum = VALUES (regUserSum)");
		log.info(sb.toString());
		String rets = dao.excuteSql(sb.toString(), null) + "";
		int ret = Integer.valueOf(rets);
		log.info("rets:" + rets);
		...
		sb.append(
				"INSERT INTO channel_operate_day ( date, registChannel, bindBankUserSum ) SELECT z.date, z.registChannel, count(s.users_id) bindBankUserSum "
						+ " FROM "
						+ " ( SELECT DATE_FORMAT(x.insert_time, '%Y-%m-%d') date, y.channel registChannel FROM dateday x, plat_channel y) z "
						+ " LEFT JOIN "
						...
						+ " ON z.date = s.date AND z.registChannel = s.registChannel GROUP BY z.date, z.registChannel "
		...
		sb = new StringBuffer();
		sb.append(
				" INSERT INTO channel_operate_day ( DATE, registChannel, activateUserSum ) SELECT z.date, z.registChannel, IFNULL(s.activateUserSum, 0) activateUserSum " +
				" FROM " +
				" ( SELECT DATE_FORMAT(X.insert_time, '%Y-%m-%d') DATE, y.channel registChannel FROM dateday X, plat_channel Y  ) z " +
				" LEFT JOIN " +
				...
				" ON z.date = s.date AND z.registChannel = s.registChannel ON DUPLICATE KEY UPDATE activateUserSum = VALUES (activateUserSum)");
		log.info(sb.toString());
		rets = dao.excuteSql(sb.toString(), null) + "";
		ret = Integer.valueOf(rets);
		log.info("rets:" + rets);
	...
	public void addChannelOperateRecharge(String yesterdays) {
		...
		sb.append(
				"INSERT INTO channel_operate_day ( date, registChannel, rechargeCount, rechargeUserSum, rechargeCentSum, avgRechargeCentSum ) "
						...
						+ "SELECT DATE_FORMAT(x.insert_time, '%Y-%m-%d') date, y.channel registChannel FROM dateday x, plat_channel y ) z LEFT JOIN "
						...
						+ "ON z.date = s.date AND z.registChannel = s.registChannel ON DUPLICATE KEY UPDATE rechargeCount = VALUES (rechargeCount), "
	...
	public void addChannelOperateWithdraw(String yesterdays) {
		...
		sb.append(
				"INSERT INTO channel_operate_day ( date, registChannel, withdrawCount, withdrawUserSum, withdrawCentSum ) "
						+ " SELECT z.date, z.registChannel, ifnull(s.withdrawCount, 0) withdrawCount, ifnull(s.withdrawUserSum, 0) withdrawUserSum, ifnull(s.withdrawCentSum, 0) withdrawCentSum " 
						+ " FROM ( SELECT DATE_FORMAT(x.insert_time, '%Y-%m-%d') date, y.channel registChannel FROM dateday x,plat_channel y ) z  "
						...
						+ " AND z.registChannel = s.registChannel ON DUPLICATE KEY UPDATE withdrawCount = VALUES (withdrawCount), "
		...
		sb = new StringBuffer();
		sb.append(
				" INSERT INTO channel_operate_day ( DATE, registChannel, successWithdrawCount, successWithdrawUserSum, successWithdrawCentSum )" +
				" SELECT z.date, z.registChannel, IFNULL(s.successWithdrawCount, 0) successWithdrawCount, IFNULL(s.successWithdrawUserSum, 0) successWithdrawUserSum, IFNULL(s.successWithdrawCentSum, 0) successWithdrawCentSum " +
				" FROM " +
				" ( SELECT DATE_FORMAT(x.insert_time, '%Y-%m-%d') DATE , y.channel registChannel FROM dateday X, plat_channel Y ) z " +
				" LEFT JOIN " +
				...
				" a.users_id = b.id WHERE a. STATUS = 1  GROUP BY DATE_FORMAT(a.insert_time, '%Y-%m-%d'), b.regist_channel ) s ON z.date = s.date AND z.registChannel = s.registChannel ON DUPLICATE KEY UPDATE successWithdrawCount = VALUES (successWithdrawCount), successWithdrawUserSum = VALUES (successWithdrawUserSum)");
	...
	public void addChannelOperateRepayment(String yesterdays) {
		...
		sb.append(
				" INSERT INTO channel_operate_day ( DATE, registChannel, repayPrincipalCentSum, repayInterestCentSum, repayAlreadyAllCentSum )" +
				" SELECT z.date, z.registChannel, IFNULL(s.repayPrincipalCentSum, 0) repayPrincipalCentSum, IFNULL(s.repayInterestCentSum, 0) repayInterestCentSum, IFNULL(s.repayAlreadyAllCentSum, 0) repayAlreadyAllCentSum" +
				" FROM " +
				" ( SELECT DATE_FORMAT(x.insert_time, '%Y-%m-%d') DATE, y.channel registChannel FROM dateday X, plat_channel Y ) z" +
				" LEFT JOIN " +
				...
				" GROUP BY DATE_FORMAT(a.return_time, '%Y-%m-%d'), b.regist_channel ) s ON z.date = s.date AND z.registChannel = s.regist_channel" +
