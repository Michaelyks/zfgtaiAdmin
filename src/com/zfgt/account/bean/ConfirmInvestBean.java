...
import com.zfgt.account.dao.ConfirmInvestDAO;
...
public class ConfirmInvestBean {
	...
	public String confirmPay(long usersId,String productId,long copies,double inMoney,double coupon,double bankMoney,String ip,String couponId) throws Exception{
		...
		String json = userRechargeBean.directBindPayToBuyProduct(UUID.randomUUID().toString(), (int)bankMoney, "中泰理财在线充值", "中泰理财在线充值", usersId, "www.baidu.com", ip);
...
//	}
	...
	/**新手产品专享的确认支付流程;
	 * @param us {@link Users}
	 * @param uin {@link UsersInfo}
	 * @param pro {@link Product}
	 * @param inv {@link Investors}
	 * @return
	 */
				...
				try {
					...
					try {
						...
						if(isPay){
							...
							platformBean.updateCollectMoney(inv.getCopies()*100);
							platformBean.updateFreshmanCoupon(inv.getCoupon());
							//阶段送投资券，达到500才送投资券
							if(copies>=500){
								userRechargeBean.sendHongBao(usersId, sendInsCoupon(usersId, copies), QwyUtil.addDaysFromOldDate(QwyUtil.fmyyyyMMdd.parse(QwyUtil.fmyyyyMMdd.format(new Date())), 15).getTime(), "0", 10001, "购买新手产品赠送投资券");
							}
							tm.commit(ts);
							insSucSmsRecordBean.sendIncSucSMS(usersInfo, pro, inv, inMoney, coupon);
							//用户购买了新手产品;
	...
	public String updateProduct(Product pro,double copies){
		...
		try {
			...
			System.out.println("产品剩宇余额:"+newPro.getLeftCopies());
			System.out.println("购买产品的份数:"+copies);
			if(!"0".equals(pro.getProductStatus())){
				log.info("支付失败:产品不在营销中;id: "+pro.getId());
				return "支付失败:产品不在营销中";
			}
